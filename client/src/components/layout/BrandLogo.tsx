    ...
    <div className={['brand-lockup', compact ? 'compact' : '', className].filter(Boolean).join(' ')} style={style}>
      ...
      <div className={['brand-mark', markClassName].filter(Boolean).join(' ')} aria-hidden="true">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" className="brand-mark-image" style={{ width: '100%', height: '100%' }}>
          ...
          <path d="M64 25 L92 42 V68 C92 84 79 96 64 101 C49 96 36 84 36 68 V42 Z" fill="url(#glowGrad)" stroke="#1e293b" stroke-width="1" />
          ...
          <path d="M64 50 L64 80 M48 65 L80 65 M64 45 L48 65 M64 45 L80 65 M48 65 L64 85 M80 65 L64 85" stroke="#38bdf8" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" opacity="0.8" />
        </svg>
      </div>
      ...
      {!compact && (
        <div className="brand-copy">
          ...
          {/* Updated Descriptions to match security/intel platform profiles */}
          <div className={['brand-description', descriptionClassName].filter(Boolean).join(' ')}>
            {showProductTag
              ? 'AI-powered automated security intelligence and OSINT analysis platform.'
              : 'Advanced threat intelligence and security assessment workspace'}
          </div>
        </div>
      )}
    </div>
